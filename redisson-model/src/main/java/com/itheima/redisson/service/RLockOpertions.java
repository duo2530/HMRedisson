...
public class RLockOpertions {
    ...
    public void  lock() throws InterruptedException{
        ...
        log.info("线程：{}，获得锁，锁存活时间：{}MS",Thread.currentThread().getName(),timeToLive);
        //休眠一下
        Thread.sleep(1000);
    ...
    public void  lockLaseTime() throws InterruptedException{
        ...
        long timeToLive = rLock.remainTimeToLive();
        log.info("线程：{}，获得锁，锁存活时间：{}S",Thread.currentThread().getName(),timeToLive);
        //休眠一下
        Thread.sleep(1000);
    ...
    public void  tryLock() throws InterruptedException {
        ...
        if (flag){
            long timeToLive = rLock.remainTimeToLive();
            log.info("线程：{}，获得锁，锁存活时间：{}S，加锁状态：{}",Thread.currentThread().getName(),timeToLive,flag);
            //休眠一下
            Thread.sleep(2000);
            //如果主线程未释放，且当前锁未调用unlock方法，则进入到watchDog机制
            //如果主线程未释放，且当前锁调用unlock方法，则直接释放锁
    ...
    public void  tryLockleasTime() throws InterruptedException {
        ...
        boolean flag = false;
            ...
            flag = rLock.tryLock(2,10, TimeUnit.SECONDS);
            if (flag){
                long timeToLive = rLock.remainTimeToLive();
                log.info("线程：{}，获得锁，锁存活时间：{}S，加锁状态：{}",Thread.currentThread().getName(),timeToLive,flag);
                //休眠一下
